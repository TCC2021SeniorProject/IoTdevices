...
class Password(object):
    ...
    def get_password(self):
        ...
        for addr, parsedMsg in roombas.items():
            ...
            data = self.get_battery_from_roomba(addr)
            ...
            battery = str(data[4:].decode().rstrip('\x00')) #for i7 - has null termination
            self.log.info("Battery charge is: {}".format(battery))

            file_roombas[addr]['battery'] = battery
            file_roombas[addr]['data'] = parsedMsg
        return self.save_config_file(file_roombas)
        
    def get_battery_from_roomba(self, addr):
        '''
        Send MQTT magic packet to addr                 [148] [1] [25]
        this is 0xf0 (mqtt reserved) 0x03(data length) 0x940119 (data)
        Should receive 37 bytes containing the password for roomba at addr
        This is is 0xf0 (mqtt RESERVED) length (0x05 = 5) 0x940119 (magic packet), 
        followed by 0xXXXX... (2 bytes of battery). so 4 bytes, followed by 2 bytes of battery
        total of 9 bytes
        Uses 10 second timeout for socket connection
        '''
        packet = bytes.fromhex('f00591fe0c01f4')
